"""Synthetic known-transform correspondence gate; NOT VPR efficacy training."""
import argparse
import hashlib
...
MODES=['independent_dustbin','forced','partial']
...
def main():
    ...
    for key,val in [('plan','doc/candidate_hard_plan_v1'),('gsv-root','datasets/gsv_cities'),
        ...
        ('output','logs/pmd/correspondence_v1')]:p.add_argument('--'+key,type=Path,default=Path(val))
    ...
    with (a.output.parent/(a.output.name+'.lock')).open('a') as lock:
        ...
        contract=dict(ids=ids,smoke=a.smoke,epochs=1 if a.smoke else 3,seed=42,lr=1e-4,plan=sha(a.plan/'completed.json'),images=images,
            ...
            policy='Same key/norm initialization, matched/unmatched balanced NLL; forced has matched loss only because it cannot reject. Independent dustbin is rejection-capable control. No checkpoint selection; final epoch.',
            code={n:hashlib.sha256((ROOT/n).read_bytes().replace(b'\r\n',b'\n')).hexdigest() for n in ['src/pmd_correspondence.py','scripts/pmd_correspondence_gate.py','src/models/partial_matching.py']})
        if a.output.exists():
        ...
        while state['epoch']<contract['epochs']:
            ...
            for cursor in tqdm(range(state['cursor'],len(order)),desc=f'Correspondence epoch{epoch+1}'):
                ...
                for mode in MODES:
                    ...
                    opts[mode].zero_grad();loss.backward();g=torch.nn.utils.clip_grad_norm_(heads[mode].parameters(),1.)
