...
The compact retrieval directory retains `summary.csv`,
`paired_comparisons.csv`, and `run.json`; the large `query_outcomes.csv` was
removed after the paired counts were verified and incorporated into
`doc/SEMANTIC_VPR_EXPERIMENT_SUMMARY.md`. Before interpreting the intervention,
...
After the failed screen, a completed audit tested whether the frozen RU BoQ
baseline already avoids dynamic regions. It used the same checkpoint, complete
query-union cache, transforms, and feature path as the retrieval screen, and
called BoQ with `attention_bias=None`. The one-time implementation is archived
in Git commit `ca158bd`; its verified conclusions are recorded in
...
fc_energy_slot_weights.csv          signed-FC proxy weights
...
The large per-image table, NumPy map dumps, ranking montages, and selected
per-head overlays were removed after their aggregate statistics were verified.
They can be regenerated by restoring the archived implementation above.
