# VPR 全部尝试、结果与结论总归档（含语义、视觉配对与官方复现）

更新日期：2026-09-11

**2026-09-11结论范围修订：** 动态类别负先验的旧负结果与原始数字保持不变，但它仅检验冻结RU上的VOC掩码与β=0.5晚期attention干预，不能关闭整个动态不变性方向。平均attention/面积比不是因果归因；740查询总体表现不能替代高动态覆盖分层或独立跨时验证。现已重新开放该方向的针对性诊断，代码与待运行协议见[动态不变性重新评估](DYNAMIC_INVARIANCE_REASSESSMENT.md)。下文“停止”的历史决定针对各已测具体配置；尚无新训练或新性能结果。

本文件是截至更新日期的统一结果入口：正文集中记录全部已知路线的机制、主要结果、证据边界和最终状态，其他文档及原始输出仅作为溯源附件。旧节中“下一步”“新路线”“通过后”等措辞是当时的历史设计，不代表当前继续执行；以本节总表及最新结案为准。记录来自已存归档、已下载结果及明确标注的训练机回传，不声称所有历史日志都已下载或在本机重新验证。

## 0. 全部路线总览与当前决策

除单独注明外，百分数为 MSLS-val R@1；不同骨干、增广、预算和选模口径不可直接作为纯语义消融比较。最佳 checkpoint 与最终 checkpoint 分开报告。首筛失败只说明当前实现没有达到扩展条件，不是证明长训必然失败或理论不成立。

| 编号 | 尝试/机制 | 已观察结果 | 当前归档结论 |
| --- | --- | --- | --- |
| B0 | 视觉基线复现：MixVPR、BoQ、SALAD | MixVPR 87.84%；原始 BoQ 91.22%；SALAD 91.08% | 保留；不混用为同一消融基线（第2节） |
| M1 | CLIP 全局 CLS 描述子蒸馏 | 已完成权重最佳 86.49–87.30%，低于 MixVPR 87.84% | 当前配置无增益；0.1记录不完整（3.1） |
| M2 | CLIP 单图 attention 蒸馏，含 consistency/divergence | 无语义 gate 87.57%；语义组 86.35–86.89% | 蒸馏及所测正则未改善（3.2） |
| M3 | VPR-conditioned patch reliability | 最佳86.49%；MSLS后20轮平均低于C0约0.96 pp | 当前可靠性目标不支持增益（3.3） |
| M4 | CLIP semantic-alias 难负样本选择 | aligned87.43%、random88.11%、shuffled87.30% | random小幅正结果不等于语义收益（3.4） |
| M5 | CLIP 语义差异正样本选择 | aligned87.03%；random87.57%、shuffled87.30%、student87.43% | 20轮首筛未通过（3.5） |
| R1 | DINO repeatability/uniqueness + CLIP affinity 平滑 | full89.86%、RU91.22%、shuffled91.22%、photometric90.27%（最佳） | full失败；RU保留为后续基座（第4节） |
| R2 | 同batch传播差分、硬稀疏和移除单位方差化 | 确认插值混杂、饱和与低覆盖；修正目标降低饱和 | 离线目标诊断，不是修正后训练成功（4.1） |
| D1 | 动态类别 attention 负先验 | aligned674/740；RU675；random674；shuffled671 | 没有净提升；旧缩小条件子集结果作废（第5节） |
| D2 | 原始 BoQ 动态区域 attention mass | 动态面积6.303%、attention mass6.381%，enrichment约1.012 | 未发现系统性动态过关注；仅机制审计（5.3） |
| Q1 | Query-conditioned Semantic BoQ，类别特定query bias | aligned/architecture-only最佳均675/740；night/season六项指标全持平 | 10轮首筛未通过；未运行后续控制（第7节） |
| F1 | Crop-CLS Local Semantic FiLM-BoQ | aligned673、architecture669、wrong-region670、wrong-place674、RU675 | 5轮四组；无正确语义配对优势（第8节） |
| C1 | DINO-anchor Residual CLIP Fusion | 3轮四组均675；成对干预同R@1；season仅+2/988 | Phase A无候选收益（第9节） |
| C2 | 已学CLIP残差推理增权 gamma0.5–4 | R@1/R@5不变；gamma4时aligned R@10少1例 | 增权未救回；VLAQ Phase B未实现（9.3–9.4） |
| S1 | RSCD 语义可靠性遮挡+关系一致性 | aligned673、no-mask673、uniform669，RU675 | 3轮首筛失败；不是mask/梯度未执行（第10节） |
| L1 | AG-SLRD 12类布局teacher/关系蒸馏准入 | holdout83.46%对67.77%；MSLS aligned344/740，仅补RU2例，shuffled补4例 | 布局可学习但互补性不足；Phase1未实现（第11节） |
| L2 | CC-LSA dense局部teacher + 手工pair评分 | crop定位97.01%但rank8.21<16；探索性重排377/740、纠错2/退步300 | teacher合同及Gate A未过；Gate B/C终止（第12节） |
| P1 | 轻量纯视觉pair V1 | 非零alpha0.01下675/740，纠错0/退步0，top1改4例 | 没有验证收益（第15节） |
| P2 | 轻量视觉pair V2 uniform/hard-mix | 两组校准均选alpha0，MSLS675/740 | 退回RU，不是新头启用后无损；停止（第15节） |
| P3 | 官方 Pair-VPR 推理复现/成对审计 | global639→refined693/740，纠错63/退步9 | 明确视觉正结果；相对RU净+18是跨系统比较（12.2） |
| P4 | Pair-VPR图片/GPS/局部匹配/仿射诊断 | 粗MNN和几何内点并不能可靠区分错误候选；固定10查询58对审计 | 未形成新重排增益，不扫描简单几何权重（12.2、第15节） |
| P5 | Pair100、Pair120与RU候选并集 | oracle724→725，仅q98新增可达，未实际重排 | 弱候选互补；非R@1提升（12.2） |
| A1 | SegVPR启发的共享骨干分割辅助 V1/V2 | V1 aligned最终673；V2 aligned671、VPR-only670、RU675 | 预热缓解梯度失衡但未获得整体收益（第13节） |
| G1 | RU-DINO-B + SAM-B Region-VLAD全库检索 | SAM382、SLIC465、grid488、shifted373，RU675；SAM并集oracle净0 | 适配配置失败，不等于官方SegVLAD失败（第16节） |
| G2 | 官方SegVLAD/AnyLoc，17Places缓存复现 | 406查询：AnyLoc385→SegVLAD387，纠错3/退步1 | 有小幅正结果；非MSLS、非纯mask消融（第17节） |
| G3 | SegVLAD投票封顶及三套GT诊断 | 官方GT：385/387/389；original259/262/263；revised269/273/274 | 事后敏感性结果，非独立验证的新方法（第17节） |
| G4 | 官方高容量SegVLAD全量MSLS迁移 | 预计约185GB；用户取消，相关实现已撤销 | 未得到结果，不能归为实验失败（第18节） |
| G5 | RU top20轻量SAM区域匹配 Region-Pair-Lite | SAM671、grid673、shifted674、RU675 | 运行完成，当前配置筛选未通过（第19节） |
| G6 | Region-Pair-Lite只读分数/门槛诊断 | 40可达错误中SAM仅3例正样本胜全部负样本；无门槛会纠错3/退步195 | 主要是分数判别不足；停止该配置（19.2–19.4） |
| U1 | 场景文字身份线索、原版SemVPR/SegVPR完整复现等 | 仅方案或代码阅读，没有对应完整实验结果 | 未验证，不列为失败或成功（第18节） |

**统一判断：** 本项目没有建立可重复、可归因于正确语义的 RU-BoQ 增益；这不意味着所有语义路线已经穷尽。官方 Pair-VPR 有明显视觉增益，官方 SegVLAD 在17Places有小幅方法级收益，必须保留这两项正证据。当前不再扩大上述已停止的配置，不自动启动新训练。本次归档不删除权重、缓存、日志，不改写历史实验预测。

## 1. 总结结论

最新结案（2026-09-10）：Region-Pair-Lite及只读分数诊断均已完成。SAM671/740低于RU675、grid673和shifted674；40个候选可达错误中只有3个正样本获得最高区域分数，直接按区域分数选第一名会纠错3例但退步195例。停止当前区域均值池化、粗平移一致性与固定重排组合。官方17Places SegVLAD的正结果及官方Pair-VPR的正结果保留，不能因自研适配失败而否定原方法。完整新增结案见第15–19节；以下带旧日期的段落是历史阶段结论。
...
- 当前证据否定了“冻结 RU/BoQ，仅靠 hard-class attention-logit adapter”、本项目的“稀疏 crop-CLS 蒸馏 + FiLM + RU/BoQ 联合微调”、“把冻结 CLIP 保留为推理特征流、零起点残差接入现有 RU+BoQ”、“语义只决定训练期结构化遮挡”，以及“由地点损失训练的 12 类 semantic-layout teacher 能为强 RU 提供足够关系蒸馏样本”等具体实现；尚未直接检验的是 SemVPR 的完整 dense LSA teacher + 原论文 CLS/SAA aggregation，以及语义只在 query-candidate 一对一局部对应中使用的 pair-VPR 路线。
...
## 11. AG-SLRD-BoQ：Phase 0 完成，FAIL

### 11.1 训练与 provenance

70×70 semantic-layout cache 覆盖 GSV-Cities 的 529,506 张图，固定 150→12 类映射，忽略 patch 数为 0。aligned 与 shuffled teacher 均使用 seed 42、同一 56,194/6,320 place split、P=40、K=4 和 10 epoch；两组均完成 `14040` 个 scheduled/optimizer step。两组各出现 3 次可恢复 AMP overflow，均通过 same-batch retry 完成，结果不存在漏更新或不对称数值协议。

修复前 aligned 在约第 6371 batch 中止的日志只记录 AMP 状态机缺陷，不是实验结果。正式 aligned/shuffled 均从 epoch 1 重新训练并写出 `final.pt/history.csv/run.json`。

### 11.2 Teacher 与 MSLS 结果

GSV 固定 holdout 上 aligned teacher 的 batch R@1 为 83.46%，shuffled 为 67.77%，aligned 高 15.70 pp。正确 semantic layout 因此确实可学习，但这不等于它能补足 RU。

| 变体 | R@1 | R@5 | R@10 | 首个正样本平均排名 |
| --- | ---: | ---: | ---: | ---: |
...
aligned 与 RU 的四格为：both-correct 342、RU-only 333、semantic-only 2、both-wrong 63；当前描述符的 oracle union 仅为 677/740。aligned positive rank 优于 RU 的查询只有 9/740（1.216%），而 RU 优于 aligned 的查询有 388 个。
...
| semantic-only 至少 8/740 | **FAIL：2/740** |
| teacher-better rank 至少 5% | **FAIL：1.216%** |
| aligned 补回数严格胜过两个控制 | **FAIL：aligned 2，wrong-layout 0，shuffled-teacher 4** |
| aligned positive rank 胜过两个控制 | PASS：698:42 与 283:180 |
| aligned GSV holdout 胜 shuffled | PASS：83.46% 对 67.77% |

逐 query 检查还显示，aligned 补回的 query 249 与 336 同时也被 shuffled teacher 补回；没有出现能够归因于正确 layout training 的 RU 专属补救。四路 descriptor 的行数、DB→query 顺序、checkpoint/cache/descriptor SHA、teacher mode 与 layout selection 均已通过自动 provenance 检查。
...
> 在 RU 已召回的 top-K 候选中，dense LSA 连续语义能否帮助 RGB 局部特征判断一对图是否来自同一地点？

它组合但不冒充复现以下论文原则：

- [SemVPR（ICCV 2025）](https://openaccess.thecvf.com/content/ICCV2025/html/Zhang_Efficient_Visual_Place_Recognition_Through_Multimodal_Semantic_Knowledge_Integration_ICCV_2025_paper.html)：先用 crop CLS 做 Local Semantic Alignment，得到比 vanilla CLIP raw patch 更可靠的连续局部语义；
- [StructVPR++（TPAMI 2025）](https://arxiv.org/abs/2503.06601)：显式进行图像对间的 semantic alignment，并对不可靠 pair 降权；
- [Pair-VPR（RA-L 2025）](https://arxiv.org/abs/2410.06614)：全局召回后用 learned pair classifier 重排候选；
- [R²Former（CVPR 2023）](https://openaccess.thecvf.com/content/CVPR2023/html/Zhu_R2Former_Unified_Retrieval_and_Reranking_Transformer_for_Place_Recognition_CVPR_2023_paper.html)：pair scorer 同时利用局部相关性、attention 与二维坐标；
- [EffoVPR（ICLR 2025）](https://proceedings.iclr.cc/paper_files/paper/2025/hash/6a1b224b153e55c40a6359f9c9fb9d8c-Abstract-Conference.html)：DINOv2 中间层局部特征本身就是强 reranking 基础。

第一步不是训练新网络，而是在 RU top-100 候选上做 LSA pair-sufficiency audit。对每个 query-candidate pair，只保留 DINO mutual-NN 且 LSA 语义相似度相对固定 GSV hard-negative 分布有正 margin 的局部对应；真正的“纠正”要求最佳 positive 的 pair score 胜过 top-100 内**全部** non-positive，而非只胜 RU 原 top-1。对照包括 DINO-full、100 seeds 的逐 pair edge-count/weight-matched DINO、raw-CLIP、token permutation 和确定性同城 wrong-place。只有 aligned 至少真实纠正 8 个 RU 错误、分别比确定性控制和 matched-control 的 95% 上界多至少 4 个，并使 positive candidate rank 优于 RU 的查询达到 37/740，才实现第二阶段。

通过后训练一个只接收 RGB/DINO token 的轻量 pair classifier。LSA teacher 只监督跨图 correspondence 与 pair confidence，推理时删除 CLIP；RU 仍负责全库召回，pair classifier 只重排 top-K。它是标准两阶段 VPR，而不是固定规则的后处理。若必须保持单阶段部署，则只能在 pair classifier 先证明互补性后再单独研究 listwise distillation，不能提前假定可蒸馏回 BoQ。
...
## 15. 轻量视觉配对 V1/V2 与官方 Pair-VPR 后续审计

本节补齐辅助视觉路线，以免“所有尝试”仅统计语义网络。轻量实现不等同于论文 Pair-VPR。

### 15.1 V1/V2训练、校准和停止原因

V1完成3轮：GSV selection三轮均990/1000；calibration从990提升至991，选择alpha0.01。MSLS675/740、纠错0/退步0，top1候选变化4次。tanh残差约43.42%饱和；关闭局部边改变分数但未带来净纠错，这是同模型敏感性检查，不是公平重训对照。

V2在同城双向候选上比较uniform与hard-mix，各3轮、每轮20,000次更新。uniform selection三轮均纠错0/退步0；hard-mix依次为0/0、1/4、2/7。校准时uniform各alpha无净收益，平局选0；hard-mix非零alpha均纠错0/退步1，也选0。两组MSLS因此都回到RU675/740，R@5=95.14%、R@10=96.08%。这不是“新头有效且无损”。

selection有1972/1982基线正确、仅8个可达错误；calibration有1976/1984基线正确、也仅8个可达错误。数据过于容易且双向样本相关，困难采样并未建立足够泛化证据。停止当前小型MNN/set-head继续训练、alpha扫描及语义扩展。来源：[V1/V2原归档](VISUAL_PAIR_RESULTS_ARCHIVE.md)。

### 15.2 官方模型正结果与诊断边界

官方Pair-VPR采用其独立预训练权重、DINOv2-register骨干及配对decoder，不是给RU直接装上同一重排器。MSLS图像路径去掉`train_val/`后，18,871数据库和740查询顺序及GT与本地一致。global639/740（86.35%）→refined693/740（93.65%），纠错63/退步9；refined R@5=97.16%、R@10=97.30%。对RU的配对正误差为纠错26/退步8，净+18，不可归因于单个语义或重排组件。

随后依次完成图片复查、GPS/方向元数据审计、原生局部匹配和固定10查询58对仿射诊断，未改GT。相似街景、同类建筑、不同视点可能产生错误匹配；仅有高MNN数/内点数不足以认定同一地点。固定7个远距离错误中，按原评分选出的最佳GT仅3例有更多内点；成功对照q49/q325的错误候选内点数也会超过GT。局部诊断不代表全部740查询的机制比例，不推导标注错误，也没有产生额外R@1收益。

Pair100及Pair120的候选oracle均724/740；Pair100∪RU20和补足120的并集均725，仅q98新增可达。增加候选不等于完成重排，未运行该并集上的全量真实重排。状态：官方模型结果保留；粗几何加权与并集扩展停止。

证据：[官方配对汇总](pairvpr_official_paired_audit_v1/summary.json)、[元数据审计](pairvpr_metadata_audit_v1/summary.json)、[几何审计](pairvpr_match_geometry_v1/summary.json)、[候选并集](pairvpr_candidate_union_v1/summary.json)。

## 16. Region-VLAD：RU/SAM-B适配，全库筛选失败

冻结RU训练过的DINO-B最后层280×280特征，使用SAM-B对象区域、邻域并集、VLAD及投票。32簇词典和PCA256由256张GSV训练图拟合，四组共享；与官方DINO-G layer31 value、PCA1024等配置不同。SAM提供无类别区域，不能把其结果直接叫作类别语义增益。

| 全库检索方式 | R@1正确数 | R@1 | 相对RU纠错/退步 | 净变化 |
| --- | ---: | ---: | ---: | ---: |
| RU | 675/740 | 91.22% | — | 0 |
| SAM | 382/740 | 51.62% | 1/294 | −293 |
| SLIC | 465/740 | 62.84% | 5/215 | −210 |
| Grid | 488/740 | 65.95% | 9/196 | −187 |
| Shifted SAM | 373/740 | 50.41% | 4/306 | −302 |

等预算RU10+区域10、去重后RU补齐20的候选oracle：SAM715（新增q241、丢失q339，净0）；SLIC717（净+2）；grid716（净+1）；shifted715（净0）。这只是可达性，不是融合检索成绩。区域数量匹配但面积和覆盖率不完全匹配，不能声称严格隔离所有分割变量。

SAM平均区域数9.55；全库缓存完成，`partial=False`表示不是部分缓存而非失败。无有效区域显式弃权，四组均保留图像索引；中途CUDA设备异常及空mask修复属于工程事件，不是算法失败证据。最终实际检索明显退化且SAM无候选净增益，停止此适配配置。证据：[结果JSON](region_vlad_msls_20260909_143829/summary.json)、[原实现协议](REGION_VLAD_SCREEN.md)。

## 17. 官方 SegVLAD / AnyLoc：17Places有限复现与正结果

### 17.1 范围、正式结果和公平比较边界

固定上游commit `628508d4a60c832961a2e2d8efa9eec724c2dc86`。采用作者17Places预计算SAM/DINO-G缓存，map/17places词典、order3、PCA1024；共享包缺少domain/indoor PCA，因此显式改选map分支，未把另一PCA改名冒充。sklearn1.3.0兼容作者PCA后命中统计不变。

| 方法 | R@1–5命中数（各406查询） | R@1 |
| --- | --- | ---: |
| AnyLoc | 385 / 390 / 392 / 393 / 395 | 94.83% |
| SegVLAD | 387 / 394 / 395 / 397 / 399 | 95.32% |

SegVLAD纠正243、257、291，退步237，净+2（+0.4926 pp）。这是已观察的小幅正结果，不应记为失败。不过AnyLoc是整图VLAD且无PCA，SegVLAD含分割、邻域、PCA和投票，不是纯分割消融；只复现作者缓存上的检索，不是端到端特征提取；更不是MSLS或RU基座增益。

### 17.2 案例、投票和GT敏感性

四案例可视化显示投票对应、重复区域和窗口边界值得检查，不能凭图片认定物理真值。每个查询区域对同一参考图只保留最大一次票值的事后封顶诊断，在官方GT下为389/406，相对SegVLAD纠错3/退步1；R@5却为398，低于原方法399。这不是独立验证的新算法，也没有消除不同查询区域间的重叠。

固定预测、保留三套GT的复核结果：

| GT协议 | AnyLoc R@1命中 | SegVLAD R@1命中 | 封顶投票R@1命中 | SegVLAD对AnyLoc纠错/退步 |
| --- | ---: | ---: | ---: | ---: |
| 官方索引±15窗口 | 385 | 387 | 389 | 3/1 |
| 包内original NPY | 259 | 262 | 263 | 5/2 |
| 包内revised NPY | 269 | 273 | 274 | 6/2 |

三套协议下SegVLAD的R@1均小幅胜AnyLoc，但original/revised下R@3–5不占优。绝对数值明显依赖GT定义，不能选择最有利协议报告，亦不能把不同GT下的数字视为不同训练效果。区域投票可能只返回1–5个候选，28个查询不足5个；按实际前缀计算，未填充或丢弃查询。

结论：官方方法有值得借鉴的对象区域检索思路，本次有限复现成功；小样本净增益不构成对本项目语义增益的证明。无需继续为了否定或证明它而无限追加17Places诊断。

证据：[成对结果转录与协议](SEGVLAD_PAIRED.md)、[投票诊断](segvlad_vote_diagnostic_v1/summary.json)、[三套GT结果](segvlad_gt_protocols_v1/summary.json)、[案例页面](segvlad_case_review_v1/index.html)。

## 18. 未运行、取消和仅提案的分支

- **官方高容量SegVLAD全量MSLS适配**：预计约185GB存储，用户明确因预算取消；对应实现与相关Git历史按当时授权撤销，替换为轻量Region-Pair-Lite。没有性能结果，不能写为MSLS复现失败，也不应重新执行旧命令。
- **完整SemVPR复现**：本项目只借鉴并实现过Crop-CLS/FiLM与CC-LSA等适配，未完成论文原配置的端到端复现。不能以这些适配失败推导原论文不可信或无效。
- **原版SegVPR**：本地仓库阅读启发SegAux共享骨干辅助训练；SegAux不是该仓库的严格原版复现。
- **VLAQ Phase B、AG-SLRD Phase1、CC-LSA Gate B/C及未触发的shuffled/多seed/长训**：因前阶段未过准入而没有实施或运行，不记录虚构结果。
- **场景文字身份、语义矛盾pair分支等**：保留设计讨论，没有完成对应收益验证；不自动作为下一项已授权训练。见[场景文字设计](SCENE_TEXT_IDENTITY_PAIR_VPR.md)。

网络/依赖安装失败、变量为空、断点恢复问题、缓存格式修复和预飞测试是工程记录，不应各算一种失败算法。用户中断的运行仅按实际完成记录统计，不能把恢复实验称为逐批严格相同的全新独立重复实验。

## 19. Region-Pair-Lite：轻量区域匹配及最终诊断结案

### 19.1 实现、恢复和最终检索结果

复用`visual_pair_msls_hard_mix_v2`的RU候选、描述子和原生DINO token；对top20涉及的11,570张图提取SAM-B区域，三组为SAM、同数量grid、平移半幅SAM。每图最多16区域，区域token均值再归一化；互为最近邻加粗平移共识评分。仅当RU分差≤0.02、支持数≥3、区域分数优势≥0.05时提升区域分数最高的候选。没有训练、VLAD或PCA。

提取约1小时8分，新区域数组估算813.5MiB（另加元数据）。匹配中遇到005389.npz空文件；原续跑仅检查存在的缺陷已修复。训练机18项测试通过，校验复用11,569分片、隔离保留损坏分片、仅重算1张后完成三组匹配。修复没有改变特征、掩码和评分配置。

| 组别 | R@1 | 正确数 | 纠错ID | 退步ID | top1候选变化 |
| --- | ---: | ---: | --- | --- | ---: |
| RU | 91.22% | 675/740 | — | — | 0 |
| SAM | 90.68% | 671/740 | 无 | 30、145、228、670 | 26 |
| Grid | 90.95% | 673/740 | 无 | 30、258 | 9 |
| Shifted SAM | 91.08% | 674/740 | 410 | 340、368 | 29 |

运行完成标志为true，合同SHA256为`0799a7ad32596d3a09158c625fd50750a93729b5e5ca8c85a45782def03b4471`（用户回传）。SAM其余22次top1变化没有改变正确性，不能都算误伤。错位组只是退化较小，也没有超过RU。

### 19.2 已下载的只读分数诊断

CPU读取既有分数、预测、GT和合同，复现全部排序及纠错/退步统计，不重跑SAM、不修改预测、不扫描阈值。RU65个错误中40个正确答案在top20、25个不可达，oracle715/740。

| 对40个可达错误的诊断 | SAM | Grid | Shifted |
| --- | ---: | ---: | ---: |
| 最高分正样本胜RU原top1的区域分数 | 17 | 16 | 21 |
| 最高分正样本严格胜全部负样本 | 3 | 4 | 7 |
| 存在通过固定门槛的正样本 | 2 | 2 | 1 |
| 只按区域argmax选top1：全部740查询正确数 | 483 | 481 | 427 |
| 上述无门槛诊断的纠错/退步 | 3/195 | 4/198 | 7/255 |

SAM无门槛R@1为65.27%，比RU少192个正确查询。只说明区域分数不能直接替代RU，不声称所有可能融合规则都已被穷尽。40例中最佳正样本被RU差距、支持数、优势门槛和非全局最高分阻挡的次数为26、24、25、37；原因有重叠，不能相加。

### 19.3 四个退步和两个“合格正样本”

| 退步query | RU正确第一名区域分数 | 被提升的错误候选区域分数 | 说明 |
| --- | ---: | ---: | --- |
| 30 | 0.0000 | 0.6679 | 正确候选未获匹配证据，错误候选借零分获得大优势 |
| 145 | 0.7010 | 0.9195 | 正确候选有证据，错误候选仍得更高分 |
| 228 | 0.7129 | 0.9041 | 同上，非单纯零分处理问题 |
| 670 | 0.0000 | 0.6988 | 另一个正样本0.6926也未超过该错误候选 |

零分是当前匹配流程没有获得足够证据，不等于证明不同地点；区域外观相似也不等于地点身份一致。

诊断字段`eligible_positive_blocked_by_global_argmax_query_ids`返回SAM的339、471，含义仅是“有合格正样本但最终未纠错”，不是“两例都能通过先门槛后argmax救回”：

- q339：合格正样本0.6551，合格错误候选0.6632，仍会选错。
- q471：最高错误候选0.8988未通过RU差距门槛；移除后仍有合格错误候选0.6804，高于合格正样本0.6606。

因此先筛门槛再选分数最高，也不能直接挽回这两例。这里是既有输出的只读检查，没有实际改动筛选策略。

### 19.4 最终归档结论及证据

状态：**工程运行完成 / 当前配置检索筛选未通过 / 停止扩展**。主要证据指向当前区域评分缺少可靠地点判别力，而不只是干预太小或门槛过严。均值池化可能丢失实例细节、重复结构可能制造伪匹配是合理机制解释，尚非独立因果实验证明。

不再提高权重、放宽阈值、长训或重提SAM来挽救此配置。不否定SAM本身、官方SegVLAD或所有语义VPR；仅否定将当前RU区域均值与粗几何分数作为可靠补充证据的这次实现。

已下载证据：[诊断汇总](region_pair_lite_diagnostic_v1/summary.json)、[逐query](region_pair_lite_diagnostic_v1/per_query.json)、[正确性变化案例](region_pair_lite_diagnostic_v1/changed_correctness_cases.json)、[来源哈希](region_pair_lite_diagnostic_v1/provenance.json)、[完成标志](region_pair_lite_diagnostic_v1/completed.json)。原屏幕结果、空分片修复输出由用户回传，原提取缓存仍保留训练机。实现提交`c1369ec`，恢复修复`a9c83e3`，诊断脚本`46ead62`；本次归档不执行本机模型或代码测试。

## 20. 跨路线经验与后续证据要求

1. **语义/对象相似不等于地点身份一致。** 类别预测、crop定位、布局teacher holdout可学习，只证明中间任务能学，不证明能补强RU。
2. **必须看净纠错与全部候选。** 胜过原top1不够，还须胜过其他错误候选；候选oracle和真实R@1分开记录。
3. **对照偶尔更好不等于随机更有效。** 很多错误配对组只比aligned少退化；MixVPR random确有单seed小幅正结果，但不是正确语义带来的证据。
4. **分支非零、梯度正常、完成训练是工程条件，不是效果条件。** 同样，阶段预算到达后停止不是训练崩溃；低预算负结果不证明任何长训都无效。
5. **区分配置失败与方法失败。** 官方Pair-VPR及17Places SegVLAD的正结果保留；自研轻量替代不代表复现原论文完整能力。所有语义路线并未穷尽。
6. **协议和随机性限制必须保留。** 条件full-db审计不是官方condition基准；17PlacesGT窗口不是MSLS的GT；最佳/最终checkpoint不能混报；恢复时未完全还原worker状态不能声称严格匹配。
7. **MSLS-val已反复用于开发。** 后续若有新收益，先冻结设计、预算与对照，再在未参与选择的独立数据检验，不能把开发集反复调参后的正结果当独立证据。

本次只统一归档文档，没有删除实验副产物、权重或历史代码，没有新增训练计划。后续同步、清理和新实验须分别按用户请求执行。
